...
void* Receiver()
{
    ...
    while( (read_size = recv(sock , message , DEFAULT_BUFLEN , 0)) > 0 )
    {
        ...
        if(strcmp(message, "crypto") == 0)
            ...
            publishers[4] = 1;   
        else if(strcmp(message, "pollution/nis") == 0)
            publishers[5] = 1;
            ...
            publishers[8] = 1;
        else if(strcmp(message, "weather/nis") == 0)
            publishers[9] = 1;
...
void* Sender()
{
    char message[24][DEFAULT_BUFLEN];
       
    sprintf(message[0], "%s", "sub -t \"crypto\"");
    sprintf(message[1], "%s", "sub -t \"numbers\"");
    sprintf(message[2], "%s", "sub -t \"pollution/belgrade\"");
    sprintf(message[3], "%s", "sub -t \"pollution/novisad\"");
    sprintf(message[4], "%s", "sub -t \"pollution/kragujevac\"");
    sprintf(message[5], "%s", "sub -t \"pollution/nis\"");
    sprintf(message[6], "%s", "sub -t \"weather/belgrade\"");
    sprintf(message[7], "%s", "sub -t \"weather/novisad\"");
    sprintf(message[8], "%s", "sub -t \"weather/kragujevac\"");
    sprintf(message[9], "%s", "sub -t \"weather/nis\"");
    sprintf(message[10], "%s", "sub -t \"trump\"");
    sprintf(message[11], "%s", "sub -t \"years\"");
    ...
    pthread_create(&hTimerThread, NULL, hTimerThread, 0);
    ...
    for(int i = 0; i<12; i++)
    {
        if(publishers[i] != 0)
        {
            ErrorMessage("Didn't receive messages for all subs");
            ThreadDestroyer();
        }
    }

    /*
    sprintf(message[12], "%s", "unsub -t \"crypto\"");
    ...
    sprintf(message[16], "%s", "unsub -t \"pollution/nis\"");
    sprintf(message[17], "%s", "unsub -t \"weather/belgrade\"");
    sprintf(message[18], "%s", "unsub -t \"weather/novisad\"");
    sprintf(message[19], "%s", "unsub -t \"weather/kragujevac\"");
    sprintf(message[20], "%s", "unsub -t \"weather/nis\"");
    sprintf(message[21], "%s", "unsub -t \"weather\"");
    sprintf(message[22], "%s", "unsub -t \"trump\"");
    sprintf(message[23], "%s", "unsub -t \"years\"");
    */
